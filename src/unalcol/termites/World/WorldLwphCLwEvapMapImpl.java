 ...
 */
public class WorldLwphCLwEvapMapImpl extends World {
    ...
    public WorldLwphCLwEvapMapImpl(Vector<Agent> agents, int w, int h) {
        super(agents, w, h);
        width = w;
        System.out.println("w" + w);
        height = h;
        GaussianGenerator g = new GaussianGenerator(0, 0.4);
    ...
    public boolean act(Agent agent, Action action) {
        ...
        System.out.println("min: (" + t.getMinx() + "," + t.getMiny());
        System.out.println("max: (" + t.getMaxx() + "," + t.getMaxy());
        int[] x = calculateNearestBlancPoint(t);
        
        System.out.print(x[0] + "," + x[1]);
        System.out.println("mov"+ getMovement(t, x));
        ...
        if (AppMain.walls.equals("wallsoff")) {
            executed = updatePiecesInWorld(t, getMovement(t, x));//this.getActionIndex(act));
        } else {
            executed = updatePiecesInWorldNotToroidal(t, getMovement(t, x));//this.getActionIndex(act));
        }
        DrawPieceInWorld(t);
        //showMap();
        return executed;
     ...
     * @return
     */
    public int[] calculateNearestBlancPoint(Termite t) {
        int[] pos = new int[2];
        int move = -1;
        double distance = 0;
        ...
        for (int i = t.getMinx(); i < t.getMaxx(); i++) {
            for (int j = t.getMiny(); j < t.getMaxy(); j++) {
                loc = i + "-" + j;
                if (!((Hashtable) t.getAttribute("inf_i")).containsKey(loc)) {
                    distance = Math.sqrt((t.getX() - i) * (t.getX() - i) + (t.getY() - j) * (t.getY() - j));
        ...
        move = getMovement(t, pos);

        System.out.println("pos" + pos[0] + "," + pos[1]);
        return pos;
    }

    private int getMovement(Termite t, int[] pos) {
        ...
        for (int j = 0; j < agentPerceptVector.length; j++) {
            distance = Math.sqrt(((piece.getX() + xs[j]) - pos[0]) * ((piece.getX() + xs[j]) - pos[0]) + (t.getY()+ys[j] - pos[1]) * (t.getY()+ys[j] - pos[1]));
            if (distance < minDistance) {
                mov = j;
